...
class Main
  ...
  def new_route
	 	first_station_name = ask('Укажите первую станцию маршрута')
	 	first_station = Station.new(first_station_name)
	 	last_station_name = ask('Укажите последнюю станцию маршрута')
	...
	def set_route
		...
		first_station2 = ask('Введитете наименование первой станции')
		last_station2 = ask('Введитете наименование конечной станции')
	...
	def wagons_list
		...
		train(id).wagons_of_train do |wagon|
		  if wagon.type == :passenger
			  puts "Пассажирский вагон №#{wagon.number}. Количество свободных мест:#{wagon.free_space}. Количество занятых мест #{wagon.occupied_space}"
		  else
			  puts "Грузовой вагон №#{wagon.number}. Количество свободного объема:#{wagon.free_space}. Количество занятого объема #{wagon.occupied_space}"
		  end
		end
  end
