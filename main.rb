...
class Main
  ...
  def new_route
    ...
    @routes << route
    puts "Маршрут создан. Начальная станция #{first_station_name}, конечная станция #{last_station_name}"
  rescue RuntimeError => e
    puts e.message
    retry
  end
  ...
  def wagons_list
    ...
    train(id).wagons_of_train do |wagon|
      if wagon.type == :passenger
        puts "Пассажирский вагон №#{wagon.number}. Свободно: #{wagon.free_space} мест(о), занято #{wagon.occupied_space} мест(о)"
      else
